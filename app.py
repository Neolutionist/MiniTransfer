...
# (Shortened headers to keep message size; identical to previous version except JS uploader below.)
LOGIN_HTML = """
...
<script>
  ...
  // PUT 1 part — met fallback timer voor voortgang (voor het geval onprogress niet vuurt)
  function putPart(url, blob, updateCb, partNumber){
    return new Promise((resolve,reject)=>{
      ...
      let last = 0;
      // Fallback ticker: update elke 250ms een klein tikje (wordt overschreven door echte progress)
  ...
  // Multipart upload — veel kleinere parts + progress robuust
  async function multipartUploadOne(token, file, relpath, expiryDays, password, totalTracker){
    const CHUNK = 1 * 1024 * 1024; // 1 MB parts → veel vaker voortgang
    const CONCURRENCY = 1;
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    const totalBytes = Math.max(1, files.reduce((a,f)=>a+f.size,0)); // nooit 0
...
@app.route("/mpu_sign", methods=["POST"])  # legacy path safeguard (not used)
def mpu_sign_legacy():
    return mpu_sign()
...
def init_db():
    c = db()
    # pakket = 1 downloadlink, kan 1 of meer files bevatten
    c.execute("""
      CREATE TABLE IF NOT EXISTS packages (
        token TEXT PRIMARY KEY,
        expires_at TEXT NOT NULL,
