...
(() => {
  ...
  /* ===== Stability rewrite patch: audio/tab/enemy sanitation ===== */
  (() => {
    const stability = {
      hiddenPaused: false,
      activeAudio: new Set(),
    ...
    function safeRemove(obj){
      try{ if(obj && obj.parent) obj.parent.remove(obj); }catch(_e){}
      try{ scene?.remove?.(obj); }catch(_e){}
    }

    function stopTrackedAudio(){
      for(const item of Array.from(stability.activeAudio)){
        try{ item.gain?.gain?.cancelScheduledValues?.(0); }catch(_e){}
        try{ item.gain?.gain?.setValueAtTime?.(0.0001, audioCtx?.currentTime || 0); }catch(_e){}
        try{ item.src?.stop?.(0); }catch(_e){}
        try{ item.osc?.stop?.(0); }catch(_e){}
        try{ item.src?.disconnect?.(); }catch(_e){}
        try{ item.osc?.disconnect?.(); }catch(_e){}
        try{ item.filter?.disconnect?.(); }catch(_e){}
        try{ item.gain?.disconnect?.(); }catch(_e){}
        stability.activeAudio.delete(item);
      }
    ...
    function trackAudio(item){
      stability.activeAudio.add(item);
      const done = () => {
        stability.activeAudio.delete(item);
        try{ item.src?.disconnect?.(); }catch(_e){}
        try{ item.osc?.disconnect?.(); }catch(_e){}
      ...
      if(item.osc) item.osc.onended = done;
      if(item.src) item.src.onended = done;
      return item;
    }

    const _ensureAudioStable = ensureAudio;
    ...
    function resetInputState(){
      ...
      input.lookX = 0;
      input.lookY = 0;
      for(const key of Object.keys(input.keyboard || {})) delete input.keyboard[key];
    }

    function sanitizeEnemy(enemy, idx){
      if(!enemy) return false;
      if(!enemy.__stableId) enemy.__stableId = `enemy_${stability.enemySeq++}`;
      if(enemy.__removed) return false;
      if(!enemy.mesh || !enemy.mesh.position) return false;
      if(!Number.isFinite(enemy.mesh.position.x) || !Number.isFinite(enemy.mesh.position.z)) return false;
      if(Math.abs(enemy.mesh.position.x) > 5000 || Math.abs(enemy.mesh.position.z) > 5000) return false;
      if(!Number.isFinite(enemy.hp)) enemy.hp = Number.isFinite(enemy.maxHp) ? enemy.maxHp : (enemy.isBoss ? 450 : enemy.type === "elite" ? 48 : 20);
      if(!Number.isFinite(enemy.maxHp)) enemy.maxHp = Math.max(1, enemy.hp);
      if(!Number.isFinite(enemy.radius) || enemy.radius <= 0) enemy.radius = enemy.isBoss ? 1.6 : 0.95;
      if(enemy.hp <= 0){
        if(enemy === state.boss){
          try{ killEnemy(enemy); }catch(_e){}
          state.boss = null;
        }else{
          try{ killEnemy(enemy); }catch(_e){}
          if(idx >= 0 && state.enemies[idx] === enemy) state.enemies.splice(idx, 1);
        }
        enemy.__removed = true;
        return false;
      }
      return true;
    }

    function sanitizeTransientList(list){
      if(!Array.isArray(list)) return;
      for(let i=list.length-1;i>=0;i--){
        const item = list[i];
        if(!item){ list.splice(i,1); continue; }
        const mesh = item.mesh || item.core || item.group;
        if(mesh?.position){
          const p = mesh.position;
          if(!Number.isFinite(p.x) || !Number.isFinite(p.y) || !Number.isFinite(p.z) || Math.abs(p.x) > 6000 || Math.abs(p.z) > 6000){
            safeRemove(item.mesh); safeRemove(item.core); safeRemove(item.group);
            list.splice(i,1);
          }
        }
        if(Number.isFinite(item.life) && item.life < -0.25){
          safeRemove(item.mesh); safeRemove(item.core); safeRemove(item.group);
          list.splice(i,1);
        }
      }
    ...
    function sanitizeGameState(){
      const now = performance.now();
      if(now - stability.lastSweep < 120) return;
      stability.lastSweep = now;

      const seenMeshes = new Set();
      for(let i=state.enemies.length-1;i>=0;i--){
        const enemy = state.enemies[i];
        if(!sanitizeEnemy(enemy, i)){
          safeRemove(enemy?.mesh);
          safeRemove(enemy?.groundRing);
          safeRemove(enemy?.shieldMesh);
          continue;
        }
        const meshId = enemy.mesh.uuid || enemy.__stableId;
      ...
      if(state.boss && !sanitizeEnemy(state.boss, -1)){
        safeRemove(state.boss?.mesh);
        safeRemove(state.boss?.groundRing);
        safeRemove(state.boss?.shieldMesh);
        state.boss = null;
      }

      sanitizeTransientList(state.bullets);
      sanitizeTransientList(state.enemyBullets);
      sanitizeTransientList(state.particles);
      sanitizeTransientList(state.pickups);
      sanitizeTransientList(state.ragdolls);
      sanitizeTransientList(state.hazards);
    }

    const _spawnEnemyStable = spawnEnemy;
    spawnEnemy = function(isBoss=false){
      const enemy = _spawnEnemyStable(isBoss);
      if(enemy){
        enemy.__removed = false;
        if(!Number.isFinite(enemy.hp)) enemy.hp = Number.isFinite(enemy.maxHp) ? enemy.maxHp : (isBoss ? 450 : enemy.type === "elite" ? 48 : 20);
        if(!Number.isFinite(enemy.maxHp)) enemy.maxHp = Math.max(1, enemy.hp);
        if(!Number.isFinite(enemy.radius) || enemy.radius <= 0) enemy.radius = isBoss ? 1.6 : 0.95;
      }
      sanitizeGameState();
      return enemy;
    };

    const _killEnemyStable = killEnemy;
    killEnemy = function(enemy){
      if(!enemy) return;
      if(enemy.__removed && enemy !== state.boss) return;
      enemy.__removed = true;
      return _killEnemyStable(enemy);
    };

    const _damageEnemyDirectStable = damageEnemyDirect;
    damageEnemyDirect = function(enemy, damage){
      if(enemy && !Number.isFinite(enemy.hp)) enemy.hp = Number.isFinite(enemy.maxHp) ? enemy.maxHp : 20;
      const result = _damageEnemyDirectStable(enemy, damage);
      sanitizeGameState();
      return result;
    };

    const _restartGameStable = restartGame;
    restartGame = function(){
      stopTrackedAudio();
      resetInputState();
      return _restartGameStable();
    };

    const _startGameStable = startGame;
    startGame = function(){
      state.lastTime = performance.now();
      sanitizeGameState();
      return _startGameStable();
    };

    function pauseForHidden(){
      stability.hiddenPaused = !!state.running;
      resetInputState();
      state.running = false;
      state.lastTime = performance.now();
      try{
    ...
    function resumeFromHidden(){
      state.lastTime = performance.now();
      if(stability.hiddenPaused && player.alive !== false) state.running = true;
      stability.hiddenPaused = false;
    ...
    window.addEventListener("blur", pauseForHidden, { passive:true });
    window.addEventListener("focus", resumeFromHidden, { passive:true });
    window.addEventListener("pagehide", pauseForHidden, { passive:true });
    window.addEventListener("pageshow", resumeFromHidden, { passive:true });

    const _animateStable = animate;
    animate = function(now){
      if(document.hidden){
        state.lastTime = now || performance.now();
        return requestAnimationFrame(animate);
      }
      sanitizeGameState();
      return _animateStable(now);
    };
  })();
