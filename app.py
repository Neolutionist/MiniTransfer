...
function updateBullets(dt){
 ...
 if(state.boss && !remove){
  ...
  if(boss.mesh){
    const bossHitPos = boss.mesh.position.clone();
    bossHitPos.y = 2.5;

    const bossRadius = Math.max(
      0.8,
      Number.isFinite(boss.radius) ? boss.radius : 2
    );
    ...
    if(b.mesh.position.distanceTo(bossHitPos) < bossRadius){
      if(b.type === "rocket" || b.type === "grenade" || b.type === "plasma"){
        ...
        const dmg = Math.max(0, Number.isFinite(b.damage) ? b.damage : 0);

        if(!Number.isFinite(boss.maxHp) || boss.maxHp <= 0){
          boss.maxHp = Number.isFinite(boss.hp) && boss.hp > 0 ? boss.hp : 1;
        }

        if(!Number.isFinite(boss.hp)){
          boss.hp = boss.maxHp;
        }
...
for(let i = state.enemyBullets.length - 1; i >= 0; i--){
  ...
  if(!remove && collidesAt(b.mesh.position.x, b.mesh.position.z, 0.14)){
    remove = true;
  }
